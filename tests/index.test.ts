...
import { APIPromise } from 'miriam-staging/core/api-promise';

import util from 'node:util';
import MiriamStaging from 'miriam-staging';
import { APIUserAbortError } from 'miriam-staging';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['MIRIAM_STAGING_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new MiriamStaging({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MIRIAM_STAGING_BASE_URL'] = 'https://example.com/from_env';
      const client = new MiriamStaging({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new MiriamStaging({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new MiriamStaging({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['MIRIAM_STAGING_API_KEY'] = 'My API Key';
    const client = new MiriamStaging();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['MIRIAM_STAGING_API_KEY'] = 'another My API Key';
    const client = new MiriamStaging({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry count header', async () => {
    ...
    const client = new MiriamStaging({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new MiriamStaging({ apiKey: 'My API Key', fetch: testFetch });
