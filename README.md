...
> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install miriam-staging`
...
1. Via the `MIRIAM_STAGING_LOG` environment variable
...
const client = new MiriamStaging({
  logger: logger.child({ name: 'MiriamStaging' }),
...
const client = new MiriamStaging({ fetch });
```
...
```ts
import MiriamStaging from 'npm:miriam-staging';
